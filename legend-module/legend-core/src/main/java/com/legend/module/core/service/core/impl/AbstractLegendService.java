...
public abstract class AbstractLegendService<T extends AbstractEntity> implements LegendService<T> {
    ...
    @Override
    public List<T> getList(T t, QueryUtils queryUtils) {
        PageHelper.startPage(queryUtils.getCurrentPage(), queryUtils.getPageSize());

        return getList(t, queryUtils.getCurrentPage(), queryUtils.getPageSize());
    }
